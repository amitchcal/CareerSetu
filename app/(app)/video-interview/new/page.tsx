...
import { Loader2, Lightbulb, FileText, Video } from 'lucide-react'
...
const TIPS = [
  'Sit in a well-lit room with a neutral background if possible.',
  'Look into the camera, not at your own face on screen.',
  'Speak naturally and clearly — the AI will transcribe and evaluate your answers.',
  'Each answer is capped at 2 minutes. The total session is 10 minutes.',
]
...
export default function VideoInterviewNewPage() {
  ...
  async function handleStart() {
    ...
    try {
      ...
      sessionStorage.setItem(`video_interview_q1_${data.sessionId}`, data.firstQuestion)
      router.push(`/video-interview/${data.sessionId}`)
    } catch (err: unknown) {
      ...
      <main className="mx-auto max-w-lg px-4 py-8 flex flex-col gap-6">
        <div>
          <div className="flex items-center gap-2 mb-1">
            ...
            <span className="text-xs font-semibold text-indigo-600 uppercase tracking-wide">Video Interview</span>
          </div>
          <h1 className="text-2xl font-bold text-gray-900">Configure your video interview</h1>
          <p className="mt-1 text-sm text-gray-500">Camera + microphone required. Answers are recorded and transcribed.</p>
        </div>
        ...
        >
          ...
          {starting ? 'Starting video interview…' : 'Start Video Interview'}
        </button>
      </main>
    </>
  )
